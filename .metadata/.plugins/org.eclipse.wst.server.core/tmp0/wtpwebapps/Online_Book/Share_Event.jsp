<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
    <%@include file="header.jsp"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<style type="text/css">
.head
{
font-size: 30px;
color: #C71585;

}
</style>
</head>
<body>
<div class="menu">
<table border="12" width="100%" height="60px">
<tr><td align="center"><a href="home.jsp" style="text-decoration: none;">Home</a></td><td align="center"><a href="about.jsp" style="text-decoration: none;">About Us</a></td><td align="center"><a href="contact.jsp" style="text-decoration: none;">Contact Us</a></td><td align="center"><a href="logout.jsp" style="text-decoration: none;">Logout</a></td>
</table>
<% HttpSession sc=request.getSession();
//sc.invalidate();
String ip=(String)sc.getAttribute("ip");



if(ip!=null)
{%>
<div class="head" align="center">
Post an event 
</div>
<% String ss=(String)request.getAttribute("msg");
     if(ss!=null)
    	 out.println(ss);
%>
<form action="EventControler" method="post" enctype="multipart/form-data">
<center><tr><td width="41%" align="right" valign="middle">Event name:</td><td ><input type="text" name="eventname" required></textarea></td></tr></center></br></br>
<center><tr><td width="41%" align="right" valign="middle">Event Description:</td><td ><textarea rows="10" cols="20" name="event_description" required></textarea></td></tr></center></br></br>

<center><tr><td width="45%" align="right" valign="middle"> upload doc file:</td><td colspan="2" align="right"><input type="file" name="file" required></td><td></td></tr></center></br></br>
<center><tr><td width="45%" align="right" valign="middle"><input type="submit" name="Submit" value="Submit"/><br><br>
</form>
 <%@include file="footer.jsp" %>
</body>

</html>
<%}

	else
	{%>
		<html>
	<body>

	<%RequestDispatcher rd=request.getRequestDispatcher("login.jsp");
	request.setAttribute("key","you are not login first you have to login");
	rd.include(request, response);
	}
%>



